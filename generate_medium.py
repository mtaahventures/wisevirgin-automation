"""
WiseVirgin Medium (2-Hour) Generator - Complete End-to-End System

Format: 1920x1080 (16:9 horizontal)
Duration: 2 hours (7200 seconds)
Verses: 240
Use Case: Standard meditation sessions
"""
import os
import sys
import asyncio
...
logger = setup_logger("medium_generator", get_daily_log_file())

# MEDIUM (2HR) CONFIGURATION
...
async def generate_medium_video(event, emotion, content_type, score=0):
    """
    Complete end-to-end 2-hour meditation video generation pipeline
    ...
    """
    logger.info("="*80)
    logger.info("WISEVIRGIN MEDIUM (2-HOUR) GENERATOR")
    logger.info(f"Event: {event} + Emotion: {emotion}")
    logger.info(f"Format: {RESOLUTION} ({ASPECT_RATIO}) | Duration: 2 hours")
    logger.info("="*80)

    output_dir = f"output/medium/{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}"
    ...
    title = f"Bible Verses for {emotion} in Face of {event} | 2HR Meditation"

    description = f"""Find peace and comfort through scripture as you face {event}.

This meditation features {len(scriptures)} carefully curated Bible verses addressing {emotion} in the context of {event}. Each verse is narrated with a gentle voice over calming nature visuals and peaceful music.
...
#BibleVerses #{emotion.replace(' ', '')} #{event.replace(' ', '')} #Meditation #ChristianMeditation #Prayer #Peace"""
    ...
    if video_id:
        ...
        logger.info(f"✅ MEDIUM VIDEO UPLOADED: {video_url}")
...
if __name__ == "__main__":
    print("\n" + "="*80)
    print("WISEVIRGIN MEDIUM (2-HOUR) GENERATOR")
    print("2-Hour Meditation Videos (1920x1080)")
    print("="*80 + "\n")
    ...
    result = asyncio.run(generate_medium_video(event, emotion, content_type, score))

    if result:
        print("\n" + "="*80)
        print("✅ MEDIUM VIDEO GENERATION COMPLETE")
        print("="*80)
