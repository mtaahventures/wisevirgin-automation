...
| FAQ Question | Documentation/Location | Keywords |
|--------------|------------------------|----------|
| Where is meditation music downloaded from? | `/root/wisevirgin/engines/production/music_downloader.py` | Internet Archive, archive.org, royalty-free music, meditation tracks |
| How do I fix FFmpeg concat path issues? | `wisevirginMTT.md` - Troubleshooting | absolute paths, Windows backslash escaping, file list format |
| Which YouTube account should I use for uploads? | `/root/wisevirgin/engines/publishing/youtube_uploader.py` | credentials_1_token.pickle, /root/youtube_tokens/, 3 accounts available |
| What music license is safe for YouTube monetization? | `/root/wisevirgin/docs/music_licensing.md` | Creative Commons, public domain, Internet Archive, attribution required |
| How do I add music attribution to video descriptions? | `/root/wisevirgin/engines/content/meditation_seo_generator.py` | description templates, "Music from Internet Archive", CC BY license |
...
| Error: Background video freezes at 15 seconds | `wisevirginMTT.md` - Afternoon Session | Multi-clip concat boundaries, use `-stream_loop -1` on single clip |
| Error: Text overlays not centered | `/root/wisevirgin/engines/production/meditation_video_assembler.py` | Mixed resolutions, force 1920x1080 scaling before processing |
| Error: FFmpeg concat creates pauses | `wisevirginMTT.md` - Decisions Made | Segment boundaries freeze, use single looped clip instead |
...
| FAQ Question | Documentation/Location | Keywords |
|--------------|------------------------|----------|
| How do I test videos before uploading to YouTube? | `wisevirginMTT.md` - Decision 4 | Frame extraction, SHA256 hash, duplicate detection, local verification |
| How do I check if a video has freezes? | `wisevirginCRI.md` - Video Testing | ffmpeg select filter, frame export, hash comparison, unique verification |
| What's the best approach for meditation videos? | `wisevirginMTT.md` - Decision 5 | Single clip looped, `-stream_loop -1`, independent layers, no concat |
| How do I make text overlays independent of background? | `wisevirginCRI.md` - Single-Clip Looping | Base video loops continuously, overlays timed separately, enable='between(t,X,Y)' |
| How do I avoid segment concatenation issues? | `/root/wisevirgin/engines/production/meditation_video_assembler.py` | Use single clip only, stream_loop infinite, no boundaries |
| How long does video assembly take? | `wisevirginMTT.md` - System Performance | ~24 seconds for 60s video (single-clip method), was ~3 minutes with concat |
| Where are test videos published? | `wisevirginMTT.md` - Test Videos | 7 iterations on YouTube, final: https://www.youtube.com/watch?v=yVpL1MdASfc |
...
**Last Updated:** 2026-01-15
**Version:** 1.2
**Status:** Updated with video freeze troubleshooting, testing methodology, and single-clip looping FAQs
